...
export default function PostCard() {
  ...
  const numberOfViews = post.sort((a, b) => b.views - a.views);
  useEffect(() => {
  ...
  return (
    <div className="w-[700px] mx-auto max-w-2xl my-8">
      {post.map((p, idx) => (
        <Link to={`${p.id}`}>
          <article
            key={idx}
            className="bg-white rounded-2xl mb-4 shadow-md overflow-hidden border border-gray-100"
          >
            <div className="flex items-start gap-4 p-4">
              <div className="flex-1">
                {p.tags.map((t, idx) => (
                  <span
                    key={idx}
                    className="text-xs border mr-2 border-gray-600 text-gray-600 px-2 rounded-lg"
                  >
                    {t}
                  </span>
                ))}
                ...
                <p className="w-full mt-3 text-sm text-gray-700 leading-relaxed">
                  {p.body}
                </p>
